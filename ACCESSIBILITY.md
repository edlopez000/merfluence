...
export, are live-region messages rather than silent no-ops. So is progress and completion —
copying the diagram as an image announces that it is copying, then that it copied, because
the export menu closes on the click and there is nothing else left on screen to say so.
...
| The export menu is not a full menu pattern | 4.1.2         | It uses `role="menu"` and closes on <kbd>Esc</kbd> or an outside click, but focus is not moved into it and arrow keys do not walk it; it is operated by <kbd>Tab</kbd> and <kbd>Enter</kbd>. Its three actions are `role="menuitem"` and its transparency option is a `role="menuitemcheckbox"` reporting `aria-checked`, so each is announced with its own role and state. Fully operable, not idiomatic. |
...
| Exported files                             | —             | A downloaded or copied PNG has no text alternative, and a downloaded SVG carries only the one the diagram had. Whoever embeds the file supplies the alternative in its new context.                                                                                                                                                                                                                        |
